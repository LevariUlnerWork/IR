...
class GlobalMethod:
    ...
    def new_words_to_query(self,term_query):

        cII = self.compute_Cij(term_query, term_query) #cII = sum of every freq^2 of the term in each doc from the local data
            ...
            for tweet_data in term_query_posting[term_query]:#[ [7,docid=1,[1,2,3,4,5,6,7],0.35436] , [1,docid=2,[1],0.4567]...]
                ...
                for term in self.tweetsTerms[doc_id]:
                    if term != term_query and term in self.inverted_index.keys():
                        ...
                        cIJ = self.compute_Cij(term_query,term)
                        cJJ = self.compute_Cij(term,term)
                        Sij = cIJ / (cII + cJJ - cIJ)
    ...
    def compute_Cij(self, term1, term2):
        ...
        for rel_docID in self.tweetsTerms.keys():

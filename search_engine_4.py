...
class SearchEngine:
    ...
    def __init__(self, config=None):
        ...
        model_dir = 'modul'
        self._model = gensim.models.KeyedVectors.load_word2vec_format(model_dir, binary=True, encoding='utf-8', unicode_errors='ignore',)
    ...
    def search(self, query):
        ...
        for q in query_list:
            try:
                newWord = self._model.most_similar([q],topn=10)
                for wordTuple in newWord:
                    if wordTuple[1] > threshold:
                        query += " " + wordTuple[0]
                    else:
                        continue
        ...
        return searcher.search(query,k=100)
    ...
    def main(self,corpus_path = "Data2/",output_path = "posting",stemming=False,queries = ["What to do"],num_docs_to_retrieve = 2000):
        ...
        with open("results.csv", 'w', newline='') as csvfile:
            ...
            for queryIndex in range(len(queries_list)):
                ...
                print(f"finished query no{queryIndex}")
